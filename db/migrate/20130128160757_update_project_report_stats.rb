...
class UpdateProjectReportStats < ActiveRecord::Migration
  def self.up
    ...
    change2 = SpProject.where(report_stats_to: 'Campus Ministry - WSN summer project')
  ...
  def self.down
    ...
    change1.update_all({report_stats_to: 'Other CCC ministry'}) if change1.present?

    change2 = SpProject.where(report_stats_to: 'Campus Ministry - Global Missions summer project')
    change2.update_all({report_stats_to: 'Campus Ministry - WSN summer project'}) if change2.present?
  end
end

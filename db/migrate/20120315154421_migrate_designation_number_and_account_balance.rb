  ...
  def self.up
    ActiveRecord::Base.connection.select_all("SELECT * FROM sp_applications WHERE designation_number <> 0 AND designation_number IS NOT NULL AND person_id IS NOT NULL").each do |row|
      ...
      if row['project_id'] || row['preference1_id'] || row['preference2_id'] || row['preference3_id'] || row['preference4_id'] || row['preference5_id']
        ...
        ActiveRecord::Base.connection.insert("INSERT INTO sp_designation_numbers (person_id, project_id, designation_number, account_balance, created_at, updated_at) VALUES ( #{row['person_id']}, #{project_id}, #{row['designation_number']}, #{row['account_balance'] || 0}, NOW(), NOW())")
      end
    end
  end

  def self.down
    ActiveRecord::Base.connection.execute("TRUNCATE sp_designation_numbers");
  end
end

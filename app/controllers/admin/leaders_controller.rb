class Admin::LeadersController < ApplicationController
  ...
  before_action :load_project, only: [:destroy, :edit, :update, :create, :add_person]
  ...
  def create
    ...
    if %w(apd pd opd coordinator).include?(params[:leader])
      @project.send(params[:leader] + '=', @person.id)
      @project.save(validate: false)
    elsif %w(staff kid evaluator volunteer non_app_participant).include?(params[:leader])

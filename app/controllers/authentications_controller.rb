class AuthenticationsController < ApplicationController
  ...
  skip_before_action :ssm_login_required, :check_authorization, except: :destroy
    ...
    if omniauth
      ...
      Authentication.transaction do
        if authentication && authentication.user
          ...
          redirect_to(authentications_url) && return
        else
          ...
          username = "#{omniauth['uid']}@#{omniauth[:provider]}" unless username.present?
          ...
          if user.save && (user.person || omniauth['info']['first_name'])
            ...
            redirect_to(new_user_url) && return
          end
        end
      end
    else
  ...
  def failed
    flash[:alert] = 'There was a problem logging you in with that method. Please try again'
    redirect_to(authentications_path) && return
  end

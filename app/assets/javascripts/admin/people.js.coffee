$ ->
  if $('#admin_people.confirm_merge input[type=submit]')[0]
    $('#admin_people.confirm_merge input[type=submit]')[0].focus()
...
$.fn.triggerPersonSearch = ->
  this.each ->
    ...
    $("#spinner_name").show()
    $.ajax
      url: '/admin/people/search_ids.json?q=' + name
      type: 'GET',
      success: (data)->
...
$.fn.triggerPersonLookup = ->
  this.each ->
    ...
    $.ajax
      url: '/admin/people/' + id + '/merge_preview?class=' + css_class
      type: 'GET',
      complete: ->

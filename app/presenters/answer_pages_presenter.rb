...
class AnswerPagesPresenter < Presenter
  def initialize_pages(answer_sheet)
    ...
    index = nil#@pages.index {|p| p.label == 'Instructions'} 
    if index
      instructions = @pages.delete_at(index)
      @pages.insert(0, instructions) if instructions
    end
    if answer_sheet.respond_to?(:project)
      if answer_sheet.project.project_specific_question_sheet && answer_sheet.project.project_specific_question_sheet.pages.first && answer_sheet.project.project_specific_question_sheet.pages.first.elements.count > 0

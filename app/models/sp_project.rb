...
class SpProject < ActiveRecord::Base
  ...
  validates_presence_of :state, if: proc { |project| !project.is_wsn? } #:is_wsn?
  ...
  def staff_and_volunteers(yr = nil)
    yr ||= year
    @volunteers ||= {}
    @volunteers[yr] ||= Person.where(id: sp_staff.where('sp_staff.year' => yr).find_all { |s| %w(Volunteer Staff).include?(s.type) }.collect(&:person_id))
                        .includes(:current_address)
                        .order('last_name, first_name')
  end
  ...
  def scholarship_chartfield
    scholarship_business_unit.to_s + '/' + scholarship_operating_unit.to_s + '/' + scholarship_department.to_s + '/' + scholarship_project.to_s
  end

  def operating_chartfield
    operating_business_unit.to_s + '/' + operating_operating_unit.to_s + '/' + operating_department.to_s + '/' + operating_project.to_s
  end
  ...
  def get_coordinates
    if country_status == 'closed'
      ...
      begin
        ...
        while SpProject.where(['latitude = ? and longitude = ?', latitude, longitude]).first
  ...
  def get_previous_year_records(_version)
    versions = SpProjectVersion.find(:all, conditions: ['sp_project_id = ? AND id IN (' + SpProject.build_search_project_id_string + ')', id], order: :year)
    versions.reverse
  end
  ...
  def initialize_project_specific_question_sheet
    unless project_specific_question_sheet
      update_attribute(:project_specific_question_sheet_id, Fe::QuestionSheet.where(label: 'Project - ' + to_s).first_or_create.id)
    end

class Activity < ActiveRecord::Base
  ...
  def get_stat_for(date, people_group = nil)
    ...
    stat_rel = stat_rel.where('peopleGroup = ?', people_group) unless people_group.blank?
    stat = stat_rel.first
    unless stat
      stat = Statistic.new
      stat.activity = self
  ...
  def get_stats_for_dates(begin_date, end_date, _people_group = nil)
    stats = nil
  ...
  def get_crs_stat_for(period_begin, period_end, _people_group = nil) # TODO: people groups
    if statistics.size > 1
      fail 'Too many stats for this Conference'
    else
  ...
  def create_in_global_registry(_base_object = nil, _relationship_name = nil)
    super(team, 'activity')
  end
